...
from nnunetv2.preprocessing.cropping.cropping import crop_to_nonzero_fdm
from nnunetv2.preprocessing.resampling.default_resampling import compute_new_shape
from nnunetv2.utilities.dataset_name_id_conversion import maybe_convert_to_dataset_name
from nnunetv2.utilities.find_class_by_name import recursive_find_python_class
from nnunetv2.utilities.plans_handling.plans_handler import PlansManager, ConfigurationManager
from nnunetv2.utilities.utils import get_filenames_of_train_images_and_targets_and_fdm


class FDMPreprocessor(object):
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], fdm: Union[np.ndarray, None], properties: dict,
        ...
        if fdm is not None:
            assert data.shape[1:] == fdm.shape[1:], "Shape mismatch between image and fracture distance map. Please fix your dataset and make use of the --verify_dataset_integrity flag to ensure everything is correct"
            fdm = np.copy(fdm)

        has_seg = seg is not None
        has_fdm = fdm is not None
        ...
        if fdm is not None:
            fdm = fdm.transpose([0, *[i + 1 for i in plans_manager.transpose_forward]])
        ...
        data, seg, fdm, bbox = crop_to_nonzero_fdm(data, seg, fdm)
        ...
        fdm = configuration_manager.resampling_fn_data(fdm, new_shape, original_spacing, target_spacing)
        if self.verbose:
    ...
    def run_case(self, image_files: List[str], seg_file: Union[str, None], fdm_file: Union[str, None], plans_manager: PlansManager,
        ...
        if fdm_file is not None:
            fdm, _ = rw.read_seg(fdm_file)
        else:
            fdm = None

        data, seg, fdm = self.run_case_npy(data, seg, fdm, data_properties, plans_manager, configuration_manager,
                                      dataset_json)
        return data, seg, fdm, data_properties

    def run_case_save(self, output_filename_truncated: str, image_files: List[str], seg_file: str, fdm_file: str,
                      plans_manager: PlansManager, configuration_manager: ConfigurationManager,
                      dataset_json: Union[dict, str]):
        data, seg, fdm, properties = self.run_case(image_files, seg_file, fdm_file, plans_manager, configuration_manager, dataset_json)
        # print('dtypes', data.dtype, seg.dtype)
        np.savez_compressed(output_filename_truncated + '.npz', data=data, seg=seg, fdm=fdm)
    ...
    def run(self, dataset_name_or_id: Union[int, str], configuration_name: str, plans_identifier: str,
        ...
        dataset = get_filenames_of_train_images_and_targets_and_fdm(join(nnUNet_raw, dataset_name), dataset_json)
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes) as p:
            ...
            for k in dataset.keys():
                r.append(p.starmap_async(self.run_case_save,
                                         ((join(output_directory, k), dataset[k]['images'], dataset[k]['label'], dataset[k]['fdm'],
