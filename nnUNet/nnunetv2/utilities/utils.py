...
def get_filenames_of_train_images_and_targets_and_fdm(raw_dataset_folder: str, dataset_json: dict = None):
    if dataset_json is None:
        dataset_json = load_json(join(raw_dataset_folder, 'dataset.json'))
    ...
    if 'dataset' in dataset_json.keys():
        ...
        images = create_lists_from_splitted_dataset_folder(join(raw_dataset_folder, 'imagesTr'),
                                                           dataset_json['file_ending'], identifiers)
        segs = [join(raw_dataset_folder, 'labelsTr', i + dataset_json['file_ending']) for i in identifiers]
        fdms = [join(raw_dataset_folder, 'fdmTr', i + dataset_json['file_ending']) for i in identifiers]
        dataset = {i: {'images': im, 'label': se, 'fdm': f} for i, im, se, f in zip(identifiers, images, segs, fdms)}
    return dataset
