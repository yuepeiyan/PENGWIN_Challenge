...
class nnUNetDataset_fdm(nnUNetDataset):
    def load_case(self, key):
        entry = self[key]
        if 'open_data_file' in entry.keys():
            data = entry['open_data_file']
        ...
        if 'open_fdm_file' in entry.keys():
            fdm = entry['open_fdm_file']
            # print('using open data file')
        elif isfile(entry['data_file'][:-4] + "_seg.npy"):
            fdm = np.load(entry['data_file'][:-4] + "_fdm.npy", 'r')
            if self.keep_files_open:
                self.dataset[key]['open_fdm_file'] = fdm
                print('saving open fdm file')
        else:
            fdm = np.load(entry['data_file'])['fdm']
        ...
        return data, seg, fdm, entry['properties']
