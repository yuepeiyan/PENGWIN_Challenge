...
def _convert_to_npy_fdm(npz_file: str, unpack_segmentation: bool = True, unpack_fdm: bool = True, overwrite_existing: bool = False,
                    verify_npy: bool = False, fail_ctr: int = 0) -> None:
    data_npy = npz_file[:-3] + "npy"
    seg_npy = npz_file[:-4] + "_seg.npy"
    fdm_npy = npz_file[:-4] + "_fdm.npy"
    try:
        npz_content = None  # will only be opened on demand
        ...
        if unpack_fdm and (overwrite_existing or not isfile(fdm_npy)):
            try:
                npz_content = np.load(npz_file) if npz_content is None else npz_content
            except Exception as e:
                print(f"Unable to open preprocessed file {npz_file}. Rerun nnUNetv2_preprocess!")
                raise e
            np.save(npz_file[:-4] + "_fdm.npy", npz_content['fdm'])
        ...
        if verify_npy:
            try:
                ...
                if isfile(seg_npy):
                    np.load(seg_npy, mmap_mode='r')
                if isfile(fdm_npy):
                    np.load(fdm_npy, mmap_mode='r')
            except ValueError:
                os.remove(data_npy)
                os.remove(seg_npy)
                os.remove(fdm_npy)
                print(f"Error when checking {data_npy} and {seg_npy} and {fdm_npy}, fixing...")
                if fail_ctr < 2:
                    _convert_to_npy(npz_file, unpack_segmentation, overwrite_existing, verify_npy, fail_ctr+1)
                else:
                    raise RuntimeError("Unable to fix unpacking. Please check your system or rerun nnUNetv2_preprocess")
...
def unpack_dataset_fdm(folder: str, unpack_segmentation: bool = True, unpack_fdm: bool = True, overwrite_existing: bool = False,
                   num_processes: int = default_num_processes,
                   verify_npy: bool = False):
    """
    all npz files in this folder belong to the dataset, unpack them all
    ...
    with multiprocessing.get_context("spawn").Pool(num_processes) as p:
        npz_files = subfiles(folder, True, None, ".npz", True)
        p.starmap(_convert_to_npy_fdm, zip(npz_files,
                                       [unpack_segmentation] * len(npz_files),
                                       [unpack_fdm] * len(npz_files),
                                       [overwrite_existing] * len(npz_files),
                                       [verify_npy] * len(npz_files))
                  )
...
if __name__ == '__main__':
    unpack_dataset_fdm('/data/ypy/dataset/nnUNet_datasets/nnUNet_preprocessed/Dataset200_Sacrum/test')