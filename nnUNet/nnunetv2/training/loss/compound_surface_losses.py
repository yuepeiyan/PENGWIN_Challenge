...
class GenSurfLoss(nn.Module):
    ...
    def forward(self, y_true, y_pred, dtm, alpha):
        ...
        y_true = get_one_hot(y_true, y_pred.shape[1])
        y_pred = softmax(y_pred, dim=1)
        dtm = dtm.squeeze(1).permute(0, 4, 1, 2, 3)
        ...
        return region_loss, boundary_loss, alpha * region_loss + (1. - alpha) * boundary_loss


class MyGenSurfLoss(nn.Module):
    def __init__(self, class_weights=None):
        super(MyGenSurfLoss, self).__init__()
        self.region_loss = DiceCELoss()  # Assuming DiceCELoss is defined elsewhere
    ...
    def forward(self, y_pred, y_true, dtm, alpha):
        # Compute region based loss
        region_loss = self.region_loss(y_true, y_pred)
        if alpha == 0:
            return region_loss
            ...
            # Prepare inputs
            y_true_one_hot = get_one_hot(y_true, y_pred.shape[1])  # Assuming get_one_hot is defined elsewhere
            y_pred = F.softmax(y_pred, dim=1)

            if self.class_weights is None:
                class_weights = torch.sum(y_true_one_hot, dim=self.axes)
                class_weights = 1. / (torch.square(class_weights) + 1.)
            else:
                class_weights = self.class_weights

            # Compute loss
            num = torch.sum(torch.square(dtm * (1 - (y_true_one_hot + y_pred))), dim=self.axes)
            num *= class_weights

            den = torch.sum(torch.square(dtm), dim=self.axes)
            den *= class_weights
            den += self.smooth

            boundary_loss = torch.sum(num, dim=1) / torch.sum(den, dim=1)
            boundary_loss = torch.mean(boundary_loss)
            boundary_loss = 1. - boundary_loss

            # return region_loss, boundary_loss, alpha * region_loss + (1. - alpha) * boundary_loss
            return alpha * boundary_loss + (1. - alpha) * region_loss

    def compute_dtm(self, y_true_one_hot):
        device = y_true_one_hot.device
        dtm = torch.zeros_like(y_true_one_hot, dtype=torch.float32)
        small_constant = 1e-5  # Small constant to ensure non-zero dtm

        for b in range(y_true_one_hot.shape[0]):
            for i in range(y_true_one_hot.shape[1]):
                mask = y_true_one_hot[b, i].cpu().numpy().astype(np.bool_)
                if np.any(mask):  # Check if mask is not all zeros
                    dtm_pos = distance_transform_edt(~mask)
                    dtm_neg = distance_transform_edt(mask)
                    dtm[b, i] = torch.tensor(dtm_neg - dtm_pos, dtype=torch.float32, device=device)
                else:
                    dtm[b, i] = torch.full_like(torch.tensor(mask, dtype=torch.float32), small_constant, device=device)
