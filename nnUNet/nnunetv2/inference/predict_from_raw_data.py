...
if __name__ == '__main__':
    ...
    predictor.initialize_from_trained_model_folder(
        join(nnUNet_results, 'Dataset200_Sacrum/nnUNetTrainer__nnUNetPlans__3d_fullres'),
        use_folds=(0,),
    ...
    img, props = SimpleITKIO().read_images([join(nnUNet_raw, 'Dataset200_Sacrum/imagesTr/005_sacrum_0000.nii.gz')])
    ret = predictor.predict_single_npy_array(img, props, None, None, False)
    ret = sitk.GetImageFromArray(ret.astype(np.int8))
    # img.SetSpacing(spacing)
    # img.SetDirection(direction)
    # img.SetOrigin(origin)
    sitk.WriteImage(ret, "test.nii.gz", useCompression=True)
